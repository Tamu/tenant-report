<template>
  <div class="main-content">
    ...
    <aside class="sidebar">
      ...
      <div class="p-4 flex-1 flex flex-col" style="overflow-y: auto;">
        ...
        <div class="flex-1">
          ...
          <div v-for="tenant in tenants" :key="tenant.id" class="list-item group"
            :class="{ 'active': selectedTenantId === tenant.id }" @click="selectTenant(tenant.id)">
            <div>
      ...
      <footer class="p-4 border-t" style="margin-top: auto;">
        <div class="flex">
          <a href="https://github.com/Tamu/tenant-report/" target="_blank"
    ...
    <main class="content-area">
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex gap-3">
          <DataUploader title="Initial Setup" description="Load ManagementPointData.csv" inputId="mng-upload"
            @file-loaded="loadManagementData" v-if="!isInitialized" />
      ...
      <div v-if="!isInitialized" class="card mt-8 p-12 text-center">
        ...
        <p class="text-muted mb-6 max-w-md mx-auto">
          Please upload the <b>ManagementPointData.csv</b> file first to load available management points into the
          system.
        </p>
      </div>
      ...
      <template v-else>
        <!-- Data Uploader Card -->
        <DataUploader title="Upload PPD Data" description="Load consumption data from iTM" inputId="ppd-upload"
    ...
    <TenantDialog :is-open="isDialogOpen" :tenant-id="editingTenantId" @close="closeDialog" />
  </div>
</template>
...
<script setup lang="ts">
  ...
  import {
    ...
    SettingsIcon, DownloadCloudIcon, GithubIcon
